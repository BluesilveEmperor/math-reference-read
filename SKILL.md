...
description: >-
  ...
  集成 Excalidraw 图示子技能:
  - excalidraw/：概念图、流程图与标注覆盖层 (arrow/callout/[A][B] markers)

  # excalidraw triggers
  概念图, 流程图, 架构图, diagram, flowchart, architecture, workflow, cover diagram, annotation overlay, [A][B]标记, 标注覆盖
compatibility:
  - python3 (mineru-open-sdk >= 0.2.5)
  - 配置文件: ~/.mineru/config.yaml（含 token）
  - excalidraw: playwright (excalidraw→PNG 渲染)
---
...
## 第 3-E 步（可选）：生成图示 / Optional Diagram Generation

> 若用户输入包含 `diagram`、`概念图`、`流程图`、`架构图` 等触发词，
> 或明确要求"画图"，则在报告生成后执行此步骤。

在审阅报告同目录下生成 excalidraw 概念图，覆盖以下类型：

| 触发词 | 图示类型 | 输出文件 |
|--------|---------|---------|
| `diagram` / `概念图` | 论文核心概念关系图 | `[PDF名]_图示_概念图.png` |
| `flowchart` / `流程图` | 研究方法/算法流程图 | `[PDF名]_图示_流程图.png` |
| `architecture` / `架构图` | 论文整体架构概览 | `[PDF名]_图示_架构图.png` |
| `annotation overlay` / `[A][B]` | 从 PDF 提取图表叠加标注 | `[PDF名]_图示_标注.png` |

基于 `excalidraw/` 子技能绘制 `.excalidraw` 源文件后渲染为 PNG。

```bash
SKILL_DIR="$(dirname "$(realpath "$0")")"
python "$SKILL_DIR/excalidraw/references/render_excalidraw.py" \
  "$output_dir/${pdf_name}_图示_*.excalidraw" \
  --output "$output_dir/${pdf_name}_图示_*.png"
```
...
## 第 4-B 步（可选）：生成封面图 / Optional Cover Diagram

> 若用户输入包含 `cover`、`封面`、`概览` 等触发词，
> 或明确要求"封面图"，则在导出后执行此步骤。

在审阅报告同目录下生成 excalidraw 封面概览图（论文方法概览 + 关键注释）。

```bash
python "$SKILL_DIR/excalidraw/references/render_excalidraw.py" \
  "$output_dir/${pdf_name}_封面图.excalidraw" \
  --output "$output_dir/${pdf_name}_封面图.png"
```
...
本 skill 集成了一个 Excalidraw 图示子技能 (`excalidraw/`)，位于 `math-reference-read/` 目录下，可作为独立 skill 被调用，也可在报告生成后增强视觉呈现。
...
- **excalidraw** 在 Step 3-E（可选）中根据触发词生成概念图/流程图/架构图；
  在 Step 4-B（可选）中生成封面概览图。
  可通过独立 skill 加载直接绘制任意图示。

### 调用方式

通过 skill 路由加载 `excalidraw/` 子技能后自动读取对应的 `references/`，无需额外配置。
如需渲染 `.excalidraw` 为 PNG，执行 `excalidraw/references/render_excalidraw.py`。
